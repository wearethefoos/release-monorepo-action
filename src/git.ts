 ...
 */
function checkoutExtraHeaderKey(): string {
  const serverUrl = process.env.GITHUB_SERVER_URL ?? 'https://github.com'
  return `http.${serverUrl}/.extraheader`
}

interface ExtraHeaderEntry {
  /** Absolute path of the config FILE this value actually lives in. */
  file: string
  value: string
}
 ...
 */
function findExtraHeaderEntries(key: string, cwd?: string): ExtraHeaderEntry[] {
  const dirArgs = cwd ? ['-C', cwd] : []
  const result = execCommand(
    'git',
    [...dirArgs, 'config', '--show-origin', '--get-all', key],
  ...
  const entries: ExtraHeaderEntry[] = []
  for (const line of result.stdout.split('\n')) {
    if (!line) continue
    const tabIndex = line.indexOf('\t')
    if (tabIndex === -1 || !line.startsWith('file:')) continue
    entries.push({
 ...
 */
function execPush(args: string[], cwd?: string): void {
  if (!gitAuthToken) {
  ...
  try {
    ...
    for (const entry of existing) {
      execCommand(
        'git',
        ['config', '--file', entry.file, '--add', key, entry.value],
        { allowNonZeroExit: true }
      )
    }
  }
